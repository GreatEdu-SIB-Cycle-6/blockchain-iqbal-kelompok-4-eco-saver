...
describe("EcoSaver Crowdfunding Testing", function () {
    ...
    describe("CrowdFunding Contract", function(){
        ...
        it("Campaign owner can create a request and get approved or rejected by admin", async function () {
            ...
            await expect(crowdFunding.connect(owner).getRequest(1)).to.be.revertedWithPanic(0x32);
    ...
    describe("Reward Contract", function () {
        ...
        it("Owner can add and delete reward item", async function () {
            ...
            // Number of rewards must be increase by one
            let numOfRwd = await reward.getRewardList();
            expect(numOfRwd.length).to.be.equal(1);
            ...
            // Delete existing reward from other than owner must be reverted
            await expect(reward.connect(admin1).deleteReward(0)).to.be.revertedWithCustomError;
            //Delete reward from owner must be success
            await reward.connect(owner).deleteReward(0);
            numOfRwd = await reward.getRewardList();
            expect(numOfRwd.length).to.be.equal(0);
        ...
        it("Donators can claim rewards according to the amount of their donation", async function(){
            ...
            await crowdFunding.connect(owner).approveRequest(0);
            // user2 donates to the campaign
            await crowdFunding.connect(user2).donateToCampaign(0, {value: ethers.parseEther("2")});
            ...
            await reward.connect(user2).claimReward(1,"aku");
            const user2NFT = await ecoSaverNFT.balanceOf(user2.address);
            await expect(user2NFT).to.be.equal(1);
 
            // user2 as donator claim physical reward
            await reward.connect(user2).claimReward(0,"Purwokerto");
            // the physical reward claim recorded into shipping history
            const history = await reward.connect(owner).getShippingHistory();
            expect(history.length).to.be.equal(1);
 
            /**
             * user2 needs to make another donation 
             * to make a reward claim
             * because his claim rights points have run out
             */
            await expect(reward.connect(user2).claimReward(0,"Purwokerto")).to.be.revertedWith("The amount of your donation is insufficient.");
